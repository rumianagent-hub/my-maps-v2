...
import { SkeletonUserRow } from "@/components/Skeleton";
...
function FollowersSkeleton() {
  return (
    <div className="max-w-lg mx-auto px-4 pt-24 pb-8">
      ...
      <div className="space-y-2">{[1, 2, 3].map((i) => <SkeletonUserRow key={i} />)}</div>
    </div>
  );
}
...
function FollowersContent() {
  ...
  const showSkeleton = loading && currentList.length === 0;
          ...
          {currentList.map((u) => (
            <div key={u.id} className="flex items-center gap-3 p-3.5 bg-white/[0.03] rounded-xl border border-white/[0.04] hover:bg-white/[0.06] transition-all">
              <Link href={`/user?u=${u.username}`} className="flex items-center gap-3 flex-1 min-w-0">
                <img src={u.photo_url || "/default-avatar.png"} alt="" className="w-11 h-11 rounded-xl ring-2 ring-indigo-500/10 object-cover shrink-0" loading="lazy" />
...
export default function FollowersPage() {
  return <Suspense fallback={<FollowersSkeleton />}><FollowersContent /></Suspense>;
}
