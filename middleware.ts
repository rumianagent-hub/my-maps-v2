...
const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || "https://nsytfuyxqicrrxinxtkw.supabase.co";
  ...
  if (pathname === "/post" || pathname === "/post/") {
    ...
    if (id) {
      ...
      if (p) {
        ...
        return new NextResponse(ogHtml({ title, desc: desc.slice(0, 200), image: p.photo_urls?.[0], url: `${SITE_URL}/post?id=${id}` }), { headers: { "Content-Type": "text/html;charset=UTF-8" } });
      }
    }
  }
  ...
  if (pathname === "/user" || pathname === "/user/") {
    ...
    if (username) {
      ...
      if (u) {
        ...
        return new NextResponse(ogHtml({ title, desc: desc.slice(0, 200), image: u.photo_url, url: `${SITE_URL}/user?u=${username}` }), { headers: { "Content-Type": "text/html;charset=UTF-8" } });
      }
    }
  }

  if (pathname === "/place" || pathname === "/place/") {
    ...
    if (placeId) {
      ...
      if (p) {
        return new NextResponse(ogHtml({ title: `${p.place_name}${p.city ? ` — ${p.city}` : ""} | ${SITE_NAME}`, desc: `See posts about ${p.place_name}`, image: p.photo_urls?.[0], url: `${SITE_URL}/place?id=${placeId}` }), { headers: { "Content-Type": "text/html;charset=UTF-8" } });
      }
    }
  }
...
export const config = {
  matcher: ["/post", "/post/", "/user", "/user/", "/place", "/place/"],
};
